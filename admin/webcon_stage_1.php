...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>MANAGEMENT WEBCON - STAGE 1</h1>
    </section>
...
<script type="text/javascript">
  ...
  function updateStartWebCon(flag){
    setLoader();
    APIManager.startStopCompetition(token, flag, 6)
  ...
  function fetchData(){
    setLoader();
    APIManager.fetchUploadZipTeam(token, 1)
      .then(function (response) {
        hideLoader();
        ...
        if(response.status_tahap_1 == 1){
          //start
          startStop += '<button class="btn btn-success" onclick="updateStartWebCon(2)"><i class="fa fa-play"></i>&nbsp; Start Tahap 1</button>';
        }else{
          //stop
          startStop += '<button class="btn btn-danger" onclick="updateStartWebCon(1)"><i class="fa fa-stop"></i>&nbsp; Stop Tahap 1</button>';
        }
        startStop += '</div>';
        ...
        if (statusCode == 1) {
          ...
          for(var a=0; a<data.length;a++){
            ...
            content += '<td><a href = "'+data[a].project_name+'" target = "_blink">Download Proporsal</a></td>';
            content += '<td>'+data[a].team_qualified_final_string+'</td>';
            content += '<td>'+data[a].created_at+'</td>';
            var action = '';
            if(data[a].team_qualified_final == '2'){
              action += '<button data-toggle = "modal" data-target = "#modal-view-custom-update-team" class="btn btn-success" onclick="addClassModalView(\'modal-success\',\'modal-view-custom-update-team\'); prepareUpdateStatusTeam('+data[a].team_id+',3,\'qualified_final\')">Go To Tahap 2</button>';
            ...
            // }else if(data[a].team_qualified_final == '3'){
            //   action += '<button data-toggle = "modal" data-target = "#modal-view-custom-update-team" class="btn btn-warning" onclick="addClassModalView(\'modal-success\',\'modal-view-custom-update-team\'); prepareUpdateStatusTeam('+data[a].team_id+',2,\'qualified_final\')">Back To Tahap 1</button>';
            }
